...
describe('object', () => {
  ...
  test('match against each element in array', () => {
    expect(createSatisfier({ a: 1 }).exec([{ a: 1 }, { b: 1, a: 1 }])).toBeUndefined()
  })

  test('diff mismatch property in array with proper path', () => {
    expect(createSatisfier({ a: 1 }).exec([{ a: 1 }, { b: 1 }])).toEqual([{ actual: undefined, expected: 1, path: [1, 'a'] }])
    expect(createSatisfier({ a: { b: { c: /foo/ } } }).exec([{ a: {} }, { a: { b: {} } }, { a: { b: { c: 'boo' } } }])).toEqual([
      { path: [0, 'a', 'b'], expected: { c: /foo/ }, actual: undefined },
...
describe('predicate function', () => {
  ...
  test('apply property predicate to array', () => {
    ...
    expect(s.exec([{ data: 2 }])).toEqual([{ path: [0, 'data'], expected: s.expected.data, actual: 2 }])
    expect(s.exec([{ foo: 'b' }])).toEqual([{ path: [0, 'data'], expected: s.expected.data, actual: undefined }])
  })
})
